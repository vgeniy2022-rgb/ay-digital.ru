...
import { labAchievements, labExperiments, labKnownExperimentIds } from './catalog';
import { normalizeLabState } from './storage';

test('LAB catalogue exposes the curated dynamic public experiments', () => {
  assert.equal(labExperiments.length, 5);
  assert.equal(new Set(labExperiments.map((item) => item.id)).size, labExperiments.length);
  assert.equal(new Set(labExperiments.map((item) => item.href)).size, labExperiments.length);
  assert.deepEqual(labExperiments.map((item) => item.href), ['/lab/builder', '/lab/canvas', '/lab/physics', '/lab/modern-os', '/lab/retro']);
  assert.deepEqual(labExperiments.map((item) => item.group), ['CREATE', 'CREATE', 'EXPERIMENT', 'SYSTEM', 'SYSTEM']);
  assert.equal(labKnownExperimentIds.length, 8);
});

test('achievement catalogue contains the complete Phase 4 progression', () => {
  assert.equal(labAchievements.length, 46);
  assert.equal(new Set(labAchievements.map((item) => item.id)).size, 46);
  ...
  for (const id of ['MODERN_FIRST_GAME', 'MODERN_CORE_COMPLETE', 'MODERN_BLOCKS_100', 'MODERN_BEST_LAP', 'MODERN_FARM_HARVEST', 'MODERN_FARM_SUPPLIER', 'MODERN_FARMER']) assert.ok(labAchievements.some((item) => item.id === id));
...
test('legacy LAB state migrates to version 2 without losing valid progress', () => {
  const normalized = normalizeLabState({
    version: 1,
    explored: ['game2d', 'game3d', 'os', 'retro'],
    achievements: { BROKE_THE_WEBSITE: '2026-08-28T01:00:00.000Z' },
    soundEnabled: true,
  });
  assert.equal(normalized.version, 2);
  assert.deepEqual(normalized.explored, ['game2d', 'game3d', 'os', 'retro']);
