...
export function ModernCoreShooter({ haptics, onExit, onRestart, onResult, onFullscreen }: ModernGameProps) {
  ...
  useEffect(() => {
    ...
    const down = (event: KeyboardEvent) => { if (event.key === 'Escape') { event.preventDefault(); if (!event.repeat) setPaused((value) => !value); return; } if ([' ', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) event.preventDefault(); keysRef.current.add(event.key.toLowerCase()); if (event.code === 'Space') shoot(); if (event.key.toLowerCase() === 'r') reload(); if (event.key.toLowerCase() === 'e') interact(); };
