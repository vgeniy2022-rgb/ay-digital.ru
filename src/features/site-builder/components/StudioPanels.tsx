...
import { studioRu, studioValueLabels } from '../i18n/ru';
...
export function PagesPanel({ project, onChange }: { project: SiteBuilderProject; onChange: (project: SiteBuilderProject) => void }) {
  ...
  return (
    <div className="studio-panel">
      <div className="studio-panel__heading"><div><small>Структура</small><h2>Страницы</h2></div><button type="button" onClick={add} aria-label="Создать страницу"><FilePlus2 /></button></div>
          ...
          <article className={project.activePageId === page.id ? 'is-active' : ''} key={page.id}>
            ...
            {project.activePageId === page.id ? (
              <div className="studio-page-settings">
                ...
                <label>Адрес страницы (slug)<input value={page.slug} onChange={(event) => updatePage(page.id, { slug: normalizeSlug(event.target.value) })} /></label>
                <label>SEO-заголовок<input value={page.title} onChange={(event) => updatePage(page.id, { title: event.target.value })} /></label>
                <label>Мета-описание<textarea value={page.metaDescription} onChange={(event) => updatePage(page.id, { metaDescription: event.target.value.slice(0, 220) })} /></label>
                <label>Изображение для соцсетей<select value={page.socialImageAssetId || ''} onChange={(event) => updatePage(page.id, { socialImageAssetId: event.target.value || undefined })}><option value="">Не выбрано</option>{assets?.assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></label>
                <label className="studio-check"><input type="checkbox" checked={page.noindex} onChange={(event) => updatePage(page.id, { noindex: event.target.checked })} /> Не индексировать</label>
...
export function AssetsPanel() {
  ...
  return (
    <div className="studio-panel">
      <div className="studio-panel__heading"><div><small>Медиатека</small><h2>Медиа</h2></div><ImagePlus /></div>
...
export function ThemePanel({ project, onChange }: { project: SiteBuilderProject; onChange: (project: SiteBuilderProject) => void }) {
  ...
  const colorLabels: Record<string, string> = { primary: 'Основной', secondary: 'Дополнительный', accent: 'Акцентный', background: 'Фон', surface: 'Поверхность', text: 'Текст', muted: 'Приглушённый', border: 'Граница', success: 'Успех', error: 'Ошибка' };
  return <div className="studio-panel"><div className="studio-panel__heading"><div><small>Система оформления</small><h2>Тема</h2></div><Palette /></div><div className="studio-token-list">{Object.entries(colors).map(([key, value]) => <label key={key}><span><i style={{ background: value }} />{colorLabels[key] || key}</span><input type="color" value={value} onChange={(event) => updateColor(key as keyof typeof colors, event.target.value)} /></label>)}</div><label className="studio-field">Ширина содержимого<input type="number" min="720" max="1600" value={project.theme.contentWidths.default} onChange={(event) => onChange({ ...project, theme: { ...project.theme, contentWidths: { ...project.theme.contentWidths, default: Number(event.target.value) } } })} /></label><label className="studio-field">Стиль кнопок<select value={project.theme.buttonPreset} onChange={(event) => onChange({ ...project, theme: { ...project.theme, buttonPreset: event.target.value as typeof project.theme.buttonPreset } })}><option value="solid">{studioValueLabels.solid}</option><option value="outline">{studioValueLabels.outline}</option><option value="soft">{studioValueLabels.soft}</option></select></label></div>;
}

export function createStudioPlugins(project: SiteBuilderProject, onChange: (project: SiteBuilderProject) => void): Plugin[] {
  return [
    { name: 'pages', label: studioRu.navigation.pages, icon: <FolderOpen />, render: () => <PagesPanel project={project} onChange={onChange} /> },
    { name: 'components', label: studioRu.navigation.components, icon: <Layers3 />, render: () => <div className="studio-panel studio-panel--native"><h2>Компоненты</h2><p>Используйте вкладку «Блоки»: компоненты собраны по категориям «Макет», «Базовые» и «Готовые секции».</p></div> },
    { name: 'assets', label: studioRu.navigation.assets, icon: <ImagePlus />, render: () => <AssetsPanel /> },
    { name: 'theme', label: studioRu.navigation.style, icon: <Palette />, render: () => <ThemePanel project={project} onChange={onChange} /> },
  ];
}
