...
import { isIpAssistConfigured } from './_visitorNetwork.mjs';
import { attributionConfigured } from './_trafficPolicyV3.mjs';
import { visitorRequestContext } from './_visitorRequestV3.mjs';
...
export default async function handler(request, response) {
  ...
  try {
    const result = await trackVisitorEvent(validated.value, await visitorRequestContext(request));
    ...
    return json(response, 202, { accepted: true, deduplicated: result.deduplicated, notification: result.notification, ...(result.ignored ? { ignored: result.ignored } : {}) });
  } catch {
